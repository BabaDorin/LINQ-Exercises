...
namespace LINQ.Exercises
{
    ...
    public class CombinedOperations
    {
        ...
        {
            // Isn't 'J' the only common character?

            // please edit/complete so that the test passes
            List<char> commonCharacters = TestData.People
                .Select(p => p.FirstName)
                .Aggregate((name, next) =>
                {
                    return new String(name.Intersect(next).ToArray());
                })
                .ToList();
        ...
        {
            IEnumerable<char> result = TestData.People
                .Select(p => p.FirstName)
                .Aggregate((name, next) =>
                {
                    return new String(
                        name.Where(c => next.Contains(c))
                        .ToArray()
                        );
                });
